'''
This is where my graphing functions will go. I will import these functions into the documents I use to analyze the simulation data.
'''
...
def hist1(x, title, label, size=(10,10), nbins=25, xlabel=None, ylabel=None, xlim=(None,None), yscale='linear', loc='best'):
    '''
    ...
    Inputs:
        ...
        - label (str)  : description of particles fired; appears in graph legend
    Kwargs:
        ...
        - loc (str/int): location of the graph legend; 1 is upper right, 5 is middle right, 9 is top middle
...
def scatter1(x, y, title, xlabel, ylabel, label, size=(10,10), s=1, xlim=(None,None), ylim=(None,None), yscale='linear', loc='best'):
    '''
...
def hists(xs, title, labels, size=(10,10), nbins=25, alpha=0.6, colors=None, xlabel=None, ylabel=None, xlim=(None,None), yscale='linear', loc='best'):
    '''
    This function graphs many histograms on the same plot.
    
    Inputs:
        - xs (2D array)      : 2D numpy array where each element is one dataset to be histogrammed
        - title (str)        : title of graph
        - label (list)       : description of particles fired; appears in graph legend
    Kwargs:
        - size (tuple)       : figure size of graph produced
        - nbins (int/array)  : number of bins in each histogram; can 'customize' for each individual histogram; can use output of calc_bins function
        - alpha (float/array): opacity of histogram bars; can 'customize' for each individual histogram
        - colors (array)     : color of each histogram; can 'customize' for each individual histogram
        - xlabel (str)       : x-axis label
        - ylabel (str)       : y-axis label
        - xlim (tuple)       : range of x-values to display in the graph
        - yscale (str)       : how to scale the values in each bin
    ...
    for i in range(len(xs)):
        h = plt.hist(xs[i], bins=nb[i], label=labels[i], alpha=a[i], color=cs[i])
...
def stats(x, label=None, misc=False, mins=False, maxs=False, alll=False, minlim=None, maxlim=None, nextlowest=0, nexthighest=0):
    '''
    This function prints out important statistics that give a better sense of the distribution.
    ...
    Kwargs:
        ...
        - alll (bool)      : whether or not to print all statistics; overrides False assignments
        - minlim (float)   : elements below this number are shown and investigated
        - maxlim (float)   : elements above this number are shown and investigated
        - nextlowest (int) : the (n+1)-lowest value in the distribution will be shown and enumerated
        - nexthighest (int): the (n+1)-highest value in the distribution will be shown and enumerated
    ...
    if mins==True:
        ...
        print()
        if minlim!=None:
            print('Number of elements below {}: {}'.format(minlim, x[x<minlim].size))
            print('Array of elements below {}: {}'.format(minlim, np.sort(x[x<minlim])[::-1]))
            print()
        if nextlowest!=0:
            xcp = copy.deepcopy(x)
            xcp = np.where(xcp==xcp.min(), xcp.max(), xcp)
            for i in range(nextlowest):
                print('Next lowest value is: {}'.format(xcp.min()))  #HERE
    ...
    if maxs==True:
        ...
        print()
        if maxlim!=None:
            print('Number of elements above {}: {}'.format(maxlim, x[x>maxlim].size))
            print('Array of elements above {}: {}'.format(maxlim, np.sort(x[x>maxlim])))
            print()
        if nexthighest!=0:
            xcp = copy.deepcopy(x)
